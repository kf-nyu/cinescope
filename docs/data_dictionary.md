...
Current-movie rating/votes are never included. Missing `start_year` → prior counts 0 / rating null.
...
| `known_*` / `has_known_*` | from broadcast known-person lookup |

**Known-person rule:** career aggregates over rated movies — ≥10 movies, ≥100,000 total votes, ≥7.0 mean rating.
